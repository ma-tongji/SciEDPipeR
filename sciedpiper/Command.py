...
DICT_CLEAN_TO_KEY = {Resource.CLEAN_NEVER : "NEVER",
                     ...
                     Resource.CLEAN_ALWAYS : "ALWAYS"}
DICT_JSON_CLEAN_INT = {u'never' : Resource.CLEAN_NEVER,
                       u'temp' : Resource.CLEAN_AS_TEMP,
                       'always' : Resource.CLEAN_ALWAYS}
...
class Command(Graph.Vertex):
    ...
    # Tested
    def __init__(self, str_cur_command, lstr_cur_dependencies,
                 lstr_cur_products, str_name="unnamed"):
        """
        Initializer.
    ...
    # Lightly tested
    @classmethod
    def func_dict_to_command(cls, dict_convert):
        """
        Change a dict of information to a command.
        The expected format is
        {Command.USTR_COMMAND_JSON: str_cur_command,
         Command.USTR_DEPENDENCIES_JSON: {Command.USTR_PATHS_JSON: [str_path],
                                          Command.USTR_CLEAN_JSON: [CLEAN]},
         Command.USTR_PRODUCTS_JSON: {Command.USTR_PATHS_JSON: [str_path, str_path],
                                      Command.USTR_CLEAN_JSON: [CLEAN, CLEAN]}}
    ...
    def func_set_resource_clean_level(self, lstr_file, i_level):
        ...
        * lstr_file : List of strings
                    ...
                    : If not absolute paths, will be given the current
                      working dir as their prefix.
        ...
        * return : Self
        ...
        if ((not lstr_file) or
                (i_level not in Resource.LSTR_CLEAN_LEVELS) or
                (not self.lstr_dependencies)):
            return self
    ...
    def func_get_dependencies_to_clean_level(self, i_level):
        """
        Gives the dependencies that should be cleaned at that level and
        any level less strict.

        Will never return files with a clean level NEVER

        If a path was not assigned a clean level, it is treated as the
    ...
    def func_detail(self):
        """
        Give a detailed and standardized view of the command.
        Mainly for detailed testing.
        """

        str_command = "Command: " + self.str_id
        str_dependencies = "Dependencies: " + ",".join(sorted([str(rsc_dep) for rsc_dep
                                                               in self.lstr_dependencies]))
        str_products = "Products: " + ",".join(sorted([str(rsc_prod)
                                                       for rsc_prod in self.lstr_products]))
