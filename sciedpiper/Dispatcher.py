...
class Runner:
        ...
        for str_arg_dest, str_arg_value in vars(args_name_space).items():
            if not str_arg_dest in dict_args_info[Arguments.C_STR_POSITIONAL_ARGUMENTS][Arguments.C_STR_VARIABLE_NAME] + [PipelineRunner.C_STR_JOB_SYSTEM_DEST ]:
                ...
                if (isinstance(str_arg_value, bool)):
                    ...
                    elif(not str_arg_value ==
                         dict_args_info[cur_str_flag][Arguments.C_STR_DEFAULT]):
    ...
    def func_update_command(self, str_command, args_name_space):
        ...
        command to directly call on the command line. Often this is used
        to add the command to dispatch the script that is made.
    ...
    def func_check_run(self):
        ...
        If an error, the contents of the error file are saved to the runner
        and a False is returned. If if no error, the contents of the error
        file is an empty string and a True is returned.
...
class LocalRunner(Runner):
    ...
    def func_check_run(self):
        ...
        with open(self.str_log_file, "r") as hndl_log:
            return(str_success in hndl_log.read())
...
def func_make_job_runner(str_key):
    ...
    * returns: A child implementation of a Runner or None when the keyword is
               not recongized.
             : Runner or None
    """

    if((not str_key) or (str_key.lower() not in [str_choice.lower()
                         for str_choice in C_LSTR_DISPATCH_CHOICES])):
        return None
    if str_key in [str_choice.lower() for str_choice in C_LSTR_QSUB_CHOICES]:
        return QSUBRunner()
    if str_key in [str_choice.lower() for str_choice in C_LSTR_LOCAL_CHOICES]:
        return LocalRunner()
    return None
