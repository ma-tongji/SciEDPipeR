...
# Add parent
    def test_add_parent_for_good_case(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[u\'Parent1\'];Children=[];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
    ...
    def test_add_parent_for_multiple_parents(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[u\'Parent01\', u\'Parent09\', u\'Parent10\'];Children=[];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
...
# func_remove_parent
    ...
    def test_remove_one_parent_for_multiple_parents(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[u\'Parent09\', u\'Parent10\'];Children=[];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
        cur_vertex.func_add_parent(Graph.Vertex("Parent09"))
        cur_vertex.func_add_parent(Graph.Vertex("Parent01"))
        cur_vertex.func_add_parent(Graph.Vertex("Parent10"))
...
# func_remove_child
    ...
    def test_remove_one_child_for_multiple_children(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[];Children=[u\'Child09\', u\'Child10\'];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
...
# func_add_child
    def test_add_child_for_good_case(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[];Children=[u'Child1'];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
        cur_vertex.func_add_child(Graph.Vertex("Child1"))
        str_result = cur_vertex.func_detail()
        self.func_test_equals(str_answer, str_result)
    ...
    def test_add_children_for_multiple_children(self):
        ...
        str_answer = "VERTEX{ ID=ID;Parents=[];Children=[u\'Child01\', u\'Child09\', u\'Child10\'];Type=VERTEX }"
        cur_vertex = Graph.Vertex("ID")
        cur_vertex.func_add_child(Graph.Vertex("Child09"))
        cur_vertex.func_add_child(Graph.Vertex("Child01"))
        cur_vertex.func_add_child(Graph.Vertex("Child10"))
...
# add vertex
    def test_add_vertex_with_no_parent(self):
      ...
      str_answer = "Graph:VERTEX{ ID=One;Parents=[u\'_i_am_Groot_'];Children=[];Type=VERTEX };VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[u\'One\'];Type=VERTEX }"
      cur_graph = Graph.Graph()
    ...
    def test_add_multiple_vertices(self):
      ...
      str_answer = "".join(["Graph:VERTEX{ ID=One;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX };",
                             "VERTEX{ ID=Three;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX };",
                             "VERTEX{ ID=Two;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX };",
                             "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[u'One', u'Three', u'Two'];Type=VERTEX }"])
      cur_graph = Graph.Graph()
      cur_graph.func_add_vertex(Graph.Vertex("One"))
    ...
    def test_add_vertex_with_parent_linear(self):
      ...
      str_answer = "".join(["Graph:VERTEX{ ID=One;Parents=[u'Two'];Children=[];Type=VERTEX };",
                             "VERTEX{ ID=Two;Parents=[u'_i_am_Groot_'];Children=[u'One'];Type=VERTEX };",
                             "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[u'Two'];Type=VERTEX }"])
      cur_graph = Graph.Graph()
      vtx_one = Graph.Vertex("One")
      cur_graph.func_add_vertex(vtx_one)
    ...
    def test_add_vertex_with_parents_dag(self):
      ...
      """
      str_answer = "".join([
                      "Graph:VERTEX{ ID=21;Parents=[u'_i_am_Groot_'];Children=[22, 23];Type=VERTEX };",
                      ...
                      "VERTEX{ ID=33;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX };",
                      "VERTEX{ ID=Eight;Parents=[u'Six'];Children=[];Type=VERTEX };",
                      "VERTEX{ ID=Five;Parents=[u'Three'];Children=[];Type=VERTEX };",
                      "VERTEX{ ID=Four;Parents=[u'Two'];Children=[u'Seven', u'Six'];Type=VERTEX };",
                      "VERTEX{ ID=One;Parents=[u'_i_am_Groot_'];Children=[u'Two'];Type=VERTEX };",
                      "VERTEX{ ID=Seven;Parents=[u'Four'];Children=[];Type=VERTEX };",
                      "VERTEX{ ID=Six;Parents=[u'Four'];Children=[u'Eight'];Type=VERTEX };",
                      "VERTEX{ ID=Three;Parents=[u'Two'];Children=[u'Five'];Type=VERTEX };",
                      "VERTEX{ ID=Two;Parents=[u'One'];Children=[u'Four', u'Three'];Type=VERTEX };",
                      "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[21, 33, u'One'];Type=VERTEX }"])
...
# _iter_
    ...
    def test_iter_for_one_cycle_graph(self):
      ...
      str_answer = "\n".join(sorted(["VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[21];Type=VERTEX }",
                              "VERTEX{ ID=21;Parents=[u'_i_am_Groot_'];Children=[22, 23];Type=VERTEX }",
                              "VERTEX{ ID=22;Parents=[21];Children=[24];Type=VERTEX }",
                              "VERTEX{ ID=23;Parents=[21];Children=[24];Type=VERTEX }",
                              "VERTEX{ ID=24;Parents=[22, 23];Children=[];Type=VERTEX }"]))
...
# Delete vertices
    ...
    def test_delete_vertices_in_4_vertex_graph_2_del(self):
      ...
      str_answer = str("\n".join(["VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[21];Type=VERTEX }",
                              "VERTEX{ ID=21;Parents=[u'_i_am_Groot_'];Children=[22];Type=VERTEX }",
                              "VERTEX{ ID=22;Parents=[21];Children=[];Type=VERTEX }"]))
      cur_graph = self.func_make_one_cycle_graph()
      cur_vtx = cur_graph.func_get_vertex(24)
      cur_graph.func_delete_vertex(cur_vtx)
...
# Merge Vertex
    ...
    def test_merge_vertex_empty_graph_simple_vertex(self):
      ...
      """
      str_answer = ";".join(sorted(["Graph:VERTEX{ ID=Test;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[u'Test'];Type=VERTEX }"]))
      cur_graph = Graph.Graph()
    ...
    def test_merge_vertex_empty_graph_complex_vertex(self):
      ...
      str_answer = ";".join(sorted([
                                     "Graph:VERTEX{ ID=Child1;Parents=[u'Test'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Child2;Parents=[u'Test'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Parent1;Parents=[u'_i_am_Groot_'];Children=[u'Test'];Type=VERTEX }",
                                     "VERTEX{ ID=Test;Parents=[u'Parent1'];Children=[u'Child1', u'Child2'];Type=VERTEX }",
                                     "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[u'Parent1'];Type=VERTEX }"
                                  ]))
      cur_graph = Graph.Graph()
    ...
    def test_merge_vertex_graph_simple_orthogonal_vertex(self):
      ...
      str_answer = ";".join(sorted([
                                     ...
                                     "VERTEX{ ID=24;Parents=[22, 23];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=33;Parents=[u'_i_am_Groot_'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Child1;Parents=[u'Test'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Child2;Parents=[u'Test'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Eight;Parents=[u'Six'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Five;Parents=[u'Three'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Four;Parents=[u'Two'];Children=[u'Seven', u'Six'];Type=VERTEX }",
                                     ...
                                     "VERTEX{ ID=Three;Parents=[u'Two'];Children=[u'Five'];Type=VERTEX }",
                                     "VERTEX{ ID=Two;Parents=[u'One'];Children=[u'Four', u'Three'];Type=VERTEX }",
                                     "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[21, 33, u'One', u'Parent1'];Type=VERTEX }"
                                  ]))
    ...
    def test_merge_vertex_graph_overlapping_vertex(self):
      ...
      str_answer = ";".join(sorted([
                                     "Graph:VERTEX{ ID=21;Parents=[u'_i_am_Groot_'];Children=[22, 23];Type=VERTEX }",
                                     "VERTEX{ ID=22;Parents=[21, u'Test'];Children=[24];Type=VERTEX }",
                                     "VERTEX{ ID=23;Parents=[21];Children=[24];Type=VERTEX }",
                                     "VERTEX{ ID=24;Parents=[22, 23];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=33;Parents=[u'Test'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Eight;Parents=[u'Six'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Five;Parents=[u'Three'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Four;Parents=[u'Two'];Children=[u'Seven', u'Six'];Type=VERTEX }",
                                     "VERTEX{ ID=One;Parents=[u'_i_am_Groot_'];Children=[u'Two'];Type=VERTEX }",
                                     "VERTEX{ ID=Seven;Parents=[u'Four'];Children=[];Type=VERTEX }",
                                     "VERTEX{ ID=Six;Parents=[u'Four'];Children=[u'Eight', u'Test'];Type=VERTEX }",
                                     "VERTEX{ ID=Test;Parents=[u'Six'];Children=[22, 33];Type=VERTEX }",
                                     "VERTEX{ ID=Three;Parents=[u'Two'];Children=[u'Five'];Type=VERTEX }",
                                     "VERTEX{ ID=Two;Parents=[u'One'];Children=[u'Four', u'Three'];Type=VERTEX }",
                                     "VERTEX{ ID=_i_am_Groot_;Parents=[];Children=[21, u'One'];Type=VERTEX }"
                                  ]))
