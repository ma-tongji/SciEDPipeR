...
class Graph:
    ...
    def __init__(self):
        ...
        """
        Root hold all nodes that have no parents, this allows multiple DAGs
        and singletons without loosing them.
        """
    ...
    def func_get_terminal_vertices(self):
        ...
        return [cur_vtx for cur_vtx in self
            if(not cur_vtx.func_get_children()
               and (not cur_vtx.str_id == self.root.str_id))]

    # Tested
    def func_get_vertex(self, str_id):
        """
        Return vertex given id.
        """
        if str_id:
    ...
    def func_add_vertex(self, vtx_new):
        ...
        if(not vtx_new.func_has_parent()
           and not vtx_new.str_id == self.root.str_id):
    ...
    def func_add_edge(self, vtx_parent, vtx_child):
        """
        Both parents must be valid nodes,
        if they are not in the graph they will be added.

        Adds parent and child vertices if they are not in the graph.
        If they are in the graph, uses the existing vertices with the
        vertices ID to make the edges.
        """
        # Deny invalid nodes
        ...
        if(len(vtx_cur_child.dict_vtx_parents) == 1 and
           (self.root.str_id in vtx_cur_child.dict_vtx_parents)):
    ...
    def func_merge_vertex(self, vtx_merge):
        ...
        # Give that dependency the links from this dependency
        # and then del this vtx
        # Get existing vertex
        ...
        while len(dq_cache):
            ...
            # Make sure you did not already see the child
                if not vtx_child in dict_visited:
                    ...
                    # We add it to the graph if all of it's other
                    # parents have been seen,
                    # add to cache and indicate it is visited.
                    f_store = True
                    for vtx_cur_parent in vtx_child.func_get_parents():
                        if((not vtx_cur_parent in dict_visited)
                            and (not vtx_cur_parent == vtx_cur)):
                            f_store = False
                    if f_store:
