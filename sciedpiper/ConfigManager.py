...
class ConfigManager(object):
    """
    Manages SciEDPipeR configuration files. These configuration files allow
    one to over write parameters passed into scripts. This allows different
    environmental setting to be passed in while still using an otherwise
    standard command or changing calls to the pipeline only because the script
    ...
    def _func_type_cast(self, str_type_string, str_value):
        ...
        if str_type_string == Arguments.C_STR_INT_TYPE:
            ...
            raise TypeError("".join(["ConfigManager::_func_type_cast:Unknown ",
                                     "type string. Type =",
                                     str(str_type_string)]))
    ...
    def func_normalize_argument(self, str_argument, dict_args_normalized):
        ...
        value for it is returned.
        """

        if (not str_argument) or (not str_argument in dict_args_normalized):
            raise NameError("".join(["ConfigManager::func_update_arguments:",
                                     "Could not find the entry from the config",
                                     " file in this pipeline's arguments. ",
                                     "Check to make sure the following ",
                                     "argument is in the pipeline's ",
                                     "arguments: ", str_argument]))
    ...
    def func_update_arguments(self,
        ...
        if(self.config.has_section(C_STR_RESOURCES_SECTION)):
            for str_resource_key, str_resource_value in self.config.items(C_STR_RESOURCES_SECTION):
                ...
                if str_resource_value in dict_resource_info:
                    setattr(args_parsed,
                            str_resource_variable,
                            dict_resource_info[str_resource_value])
        ...
        for str_long_flag, dict_flag_info in dict_args_info.items():
            if((Arguments.C_STR_CHOICES in dict_flag_info) and
                dict_flag_info[Arguments.C_STR_CHOICES]):
                if not getattr(args_parsed, dict_flag_info[Arguments.C_STR_VARIABLE_NAME]) in dict_flag_info[Arguments.C_STR_CHOICES]:
                    " ".join(["When using the pipeline config file, please be aware of variables with restricted values (or choices).",
    ...
    def func_get_precommands( self ):
        if(not self.config.has_section(C_STR_COMMANDS_SECTION)):
            return("")
